 ...
 * You can use Prisma enums too (if you're using them for roles), just import
 * your enum type from `@prisma/client`
 */

/**
 * Checks if the currentUser is authenticated (and assigned one of the given
 * roles)
 *
 * @param roles: {@link AllowedRoles} - Checks if the currentUser is assigned
 *   one of these roles
 *
 * @returns {boolean} - Returns true if the currentUser is logged in and
 *   assigned one of the given roles, or when no roles are provided to check
 ...
 *
 * @param roles?: {@link AllowedRoles} - When checking role membership, these
 *   roles grant access.
 *
