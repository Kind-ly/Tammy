...
describe('users', () => {
  ...
  scenario('creates a user', async () => {
    const result = await createUser({
      input: { email: 'String40981', name: 'String', role: 'String' },
    })

    expect(result.email).toEqual('String40981')
  ...
  scenario('updates a user', async (scenario) => {
    ...
    const result = await updateUser({
      id: original.id,
      input: { email: 'String41984952' },
    })

    expect(result.email).toEqual('String41984952')
  })
